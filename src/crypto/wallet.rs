...
impl Wallet {
    pub fn new() -> Wallet {
        ...
        let address = Wallet::pub_key_to_wallet_address(&pub_key)
            .expect("newly generated key must be valid hex");
        Wallet {
            pub_key,
            secret_key,
            address,
    ...
    pub fn from_keys(pub_key: &str, secret_key: &str) -> Result<Wallet> {
        let address = Wallet::pub_key_to_wallet_address(pub_key)?;
        Ok(Wallet {
    ...
    fn pub_key_to_wallet_address(pub_key: &str) -> Result<String> {
        let decoded = hex::decode(pub_key)?;
...
mod tests {
    ...
    fn test_wallet_address_generation() {
        ...
        let address = Wallet::pub_key_to_wallet_address(test_public_key).unwrap();
